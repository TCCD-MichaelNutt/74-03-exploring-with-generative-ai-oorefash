...
ntokens = 255 # the size of vocabulary (number of unique tokens)
ninp = 4*(128+4+6) # embedding dimension, calculated based on embeddings being concatenated
...
class TransformerModel(nn.Module):
    ...
    def __init__(self, ntoken, ninp, nhead, nhid, nlayers, dropout=0.5):
        ...
        self.pos_encoder = PositionalEncoding(ninp, dropout) # positional encoding module

        encoder_layers = TransformerLayer(ninp, nhead, nhid, dropout) # custom transformer encoder layer
        self.transformer_encoder = TransformerModule(encoder_layers, nlayers) # custom encoder stack

        self.encoder = nn.Embedding(ntoken, 128) # embedding for octree tokens
        self.encoder1 = nn.Embedding(MAX_OCTREE_LEVEL+1, 6) # embedding for tree levels
        self.encoder2 = nn.Embedding(9, 4) # embedding for octants

        self.ninp = ninp
        self.act = nn.ReLU() # activation function
        self.decoder0 = nn.Linear(ninp, ninp) # linear layer
        self.decoder1 = nn.Linear(ninp, ntoken) # output layer to predict next token
        self.init_weights() # initialize weights

    def generate_square_subsequent_mask(self, sz):
        # generates a square mask for the sequence (prevent attention to future tokens)
    ...
    def forward(self, src, src_mask, dataFeat):
        ...
        oct = src[:,:,:,0] # octree token
        level = src[:,:,:,1]  # tree level
        octant = src[:,:,:,2] # octant index

        # adjust levels for training max level 10
        ...
        # scale and encode with transformer
        src = a.reshape((bptt,a.shape[1],self.ninp))* math.sqrt(self.ninp)
        output = self.transformer_encoder(src, src_mask)
        output = self.decoder1(self.act(self.decoder0(output))) # project to token logits
        return output
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model, dropout=0.1, max_len=5000):
        ...
        # create constant positional encodings matrix
...
######################################################################
# Function to prepare input and target sequences
#
def get_batch(source, i):
    seq_len = min(bptt, len(source) - 1 - i)
    data = source[i:i+seq_len].clone()
    target = source[i+1:i+1+seq_len,:,-1,0].reshape(-1)
    # move octant and level features forward
...
if __name__=="__main__":
    ...
    # reload model checkpoint if available
    saveDic = None
    ...
    def train(epoch):
        ...
        for Batch, d in enumerate(train_loader): # outer batch loop
            batch = 0
